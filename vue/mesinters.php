<?php
    ...
    $sql = "SELECT * FROM intervention,client WHERE intervention.Numero_Client=client.Numero_Client AND intervention.Matricule=$matricule LIMIT $premier, $parPage;"; /* SELECTION DES DONNEES A AFFICHER SUR LA PAGE */
    $liste = mysqli_query($db, $sql);

    if (isset($_POST['dateP'])){
        /* BOUTON DE TRIAGE PAR DATE D'INTERVENTION */
        $sqldate = "SELECT * FROM intervention,client WHERE intervention.Numero_Client=client.Numero_Client AND intervention.Matricule=$matricule ORDER BY Date_Visite LIMIT $premier, $parPage;";
        $liste = mysqli_query($db, $sqldate);
    }

    if (isset($_POST['etat'])){
        /* BOUTON DE TRIAGE PAR MATRICULE D'AGENT */
        $sqlagent = "SELECT * FROM intervention,client WHERE intervention.Numero_Client=client.Numero_Client AND intervention.Matricule=$matricule ORDER BY intervention.Etat LIMIT $premier, $parPage;";
